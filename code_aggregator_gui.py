...
def find_files(directory: str, extensions: list, ignore_items: set, log_queue: queue.Queue) -> list:
    ...
    ignore_full_paths = {os.path.abspath(item).lower() for item in ignore_items if os.path.sep in item}
    ...
    for root, dirs, files in os.walk(directory, topdown=True):
        ...
        # 2. 根据完整路径过滤
        dirs[:] = [d for d in dirs if os.path.abspath(os.path.join(root, d)).lower() not in ignore_full_paths]
...
def get_unique_filepath(directory: str, filename: str, extension: str, log_queue: queue.Queue) -> str:
    """
...
def aggregate_code(root_dir: str, file_paths: list, output_filename: str, output_format: str, log_queue: queue.Queue, progress_queue: queue.Queue):
    """
    ...
    try:
        with open(output_filename, 'w', encoding='utf-8') as output_file:
            ...
            log_queue.put(f"所有代码内容已成功聚合到 '{os.path.basename(output_filename)}' 文件中。")
    except IOError as e:
...
class CodeAggregatorApp:
    ...
    def create_widgets(self, parent):
        # --- 1. 目标目录选择 ---
        dir_frame = ttk.LabelFrame(parent, text="1. 选择要提取代码的根目录", padding="10")
        dir_frame.pack(fill=tk.X, padx=5, pady=5)
        
        self.dir_path = tk.StringVar()
        ttk.Entry(dir_frame, textvariable=self.dir_path).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 5))
        ttk.Button(dir_frame, text="浏览...", command=self.select_directory).pack(side=tk.LEFT)
        ...
        self.ext_vars = {}
        common_exts = ['.py', '.js', '.html', '.css', '.c', '.h', '.cpp', '.java', '.go', '.rs']
        ...
        for i, ext in enumerate(common_exts):
            ...
            ttk.Checkbutton(ext_check_frame, text=ext, variable=var).pack(side=tk.LEFT, padx=5)

        ext_custom_frame = ttk.Frame(ext_frame, padding=(0,5))
        ext_custom_frame.pack(fill=tk.X)
        ttk.Label(ext_custom_frame, text="其他类型 (用逗号分隔):").pack(side=tk.LEFT, padx=(0,5))
        self.custom_extensions = tk.StringVar()
        self.custom_extensions_entry = ttk.Entry(ext_custom_frame, textvariable=self.custom_extensions)
        self.custom_extensions_entry.pack(fill=tk.X, expand=True)
        self._setup_placeholder()

        # --- 3. 忽略文件夹/文件 ---
        ignore_frame = ttk.LabelFrame(parent, text="3. 设置要忽略的文件夹/文件 (支持按名称或完整路径忽略)", padding="10")
        ...
        self.ignore_vars = {}
        common_ignores = ['venv', '__pycache__', '.git', '.vscode', 'node_modules', 'dist', 'build']
        for item in common_ignores:
            var = tk.BooleanVar(value=True)
            self.ignore_vars[item] = var
            ttk.Checkbutton(defaults_frame, text=item, variable=var).pack(side=tk.LEFT, padx=5)
        ...
        scrollbar = ttk.Scrollbar(list_container, orient=tk.VERTICAL, command=self.ignore_listbox.yview)
        ...
        ttk.Button(btn_frame, text="添加文件夹", command=self.add_ignore_folder).pack(pady=2, fill=tk.X)
        ttk.Button(btn_frame, text="添加文件", command=self.add_ignore_file).pack(pady=2, fill=tk.X)
        ttk.Button(btn_frame, text="移除选中项", command=self.remove_ignore_item).pack(pady=2, fill=tk.X)
        ...
        ttk.Entry(output_dir_frame, textvariable=self.output_dir_path).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 5))
        ttk.Button(output_dir_frame, text="浏览...", command=self.select_output_directory).pack(side=tk.LEFT)
        ...
        ttk.Entry(output_file_frame, textvariable=self.output_filename, width=30).pack(side=tk.LEFT)
        
        ttk.Label(output_file_frame, text="格式:").pack(side=tk.LEFT, padx=(15, 5))
        self.output_format = tk.StringVar(value=".md")
        ttk.OptionMenu(output_file_frame, self.output_format, ".md", ".md", ".txt").pack(side=tk.LEFT)
        ...
        self.save_button = ttk.Button(save_btn_frame, text="保存当前配置", command=self.save_config_manual)
        ...
        self.start_button = ttk.Button(action_frame, text="开始聚合", command=self.start_aggregation_thread)
        self.start_button.pack(pady=5)
        
        self.progress_bar = ttk.Progressbar(action_frame, orient='horizontal', mode='determinate')
        self.progress_bar.pack(fill=tk.X, pady=5)
        
        self.log_area = scrolledtext.ScrolledText(action_frame, height=10, wrap=tk.WORD, state=tk.DISABLED)
    ...
    def run_aggregation_logic(self):
        try:
            ...
            if custom_ext_str != self.placeholder_text:
                custom_exts = [ext.strip() for ext in custom_ext_str.split(',') if ext.strip()]
            ...
            ignored_items = {name for name, var in self.ignore_vars.items() if var.get()}
            custom_ignored = self.ignore_listbox.get(0, tk.END)
            ignored_items.update(custom_ignored)
            
            output_dir = self.output_dir_path.get()
            if not output_dir or not os.path.isdir(output_dir):
                self.log_queue.put(f"警告: 输出目录 '{output_dir}' 无效。将使用程序所在目录。")
            ...
            output_file_path = get_unique_filepath(output_dir, base_filename, output_format, self.log_queue)

            # 2. 执行文件查找和聚合
            found_files = find_files(target_dir, extensions, ignored_items, self.log_queue)
            
            if found_files:
                aggregate_code(target_dir, found_files, output_file_path, output_format, self.log_queue, self.progress_queue)
            ...
            while not self.log_queue.empty():
                ...
                if isinstance(message, str):
                    if message.startswith("SUCCESS:"):
                        ...
                        if messagebox.askyesno("完成", f"代码已成功聚合!\n\n文件保存在:\n'{output_file}'\n\n是否立即打开该文件？"):
                            try:
    ...
    def save_config(self):
        ...
        config = {
            ...
            'extensions_checked': {ext: var.get() for ext, var in self.ext_vars.items()},
            'extensions_custom': custom_ext_val,
            'ignore_defaults': {name: var.get() for name, var in self.ignore_vars.items()},
